# instalar nympy pip install numpy
# instalar pandas pip install pandas
# instalar opencv pip install opencv-python

import cv2  #designed for computer vision color particular pixel 
import pandas as pd # tipo matricial  2d csv particles colors 

# --------------------------------------------------------------------------

img_path = 'pic4.jpg' # puedes cambiar las imágenes
csv_path = 'colors.csv' #data frame de los colores

# reading csv file
#definiendo variables del csv
index = ['color', 'color_name', 'hex', 'R', 'G', 'B'] # tiene 5 columnas
df = pd.read_csv(csv_path, names=index, header=None)

#print(len(df)) #865
#print(df.loc[1]) # te devuele la informacion de la primera fila 
#print(df.loc[1,"R"]) # extrae el valor 0 de la segunda fila o,1

# reading image with OPENCV
img = cv2.imread(img_path)
img = cv2.resize(img, (800,620))
#print(img) pixeles [area]
#cv2.namedWindow('image')
#cv2.imshow('image',img) #se necesita reducir la imagen
#cv2.waitKey(0)
#csv2.destroyAllWindows()


# declaring global variables
clicked = False
r = g = b = xpos = ypos = 0

#function to calculate minimum distance from all colors da el color mas cercano o par
def get_color_name(R,G,B):
	minimum = 1000
	for i in range(len(df)):
		d = abs(R - int(df.loc[i,'R'])) + abs(G - int(df.loc[i,'G'])) + abs(B - int(df.loc[i,'B']))
		if d <= minimum:
			minimum = d
			cname = df.loc[i, 'color_name']

	return cname

#function to get x,y coordenadas of mouse double click
def draw_function(event, x, y, flags, params):
	if event == cv2.EVENT_LBUTTONDBLCLK: #doble click
		global b, g, r, xpos, ypos, clicked
		clicked = True
		xpos = x
		ypos = y
		b,g,r = img[y,x] #ojo con el orden
		b = int(b)
		g = int(g)
		r = int(r)

# creating window
cv2.namedWindow('image')
cv2.setMouseCallback('image', draw_function)

while True:
	cv2.imshow('image', img)
	if clicked:
		#cv2.rectangle(image, startpoint, endpoint, color, thickness)-1 fills entire rectangle posiciones
		cv2.rectangle(img, (20,20), (600,60), (b,g,r), -1)

		#Creating text string to display( Color name and RGB values )
		text = get_color_name(r,g,b) + ' R=' + str(r) + ' G=' + str(g) + ' B=' + str(b)
		#cv2.putText(img,text,start,font(0-7),fontScale,color,thickness,lineType )
		cv2.putText(img, text, (50,50), 2,0.8, (255,211,0),2,cv2.LINE_AA)

		#For very light colours we will display text in black colour
		if r+g+b >=600:
			cv2.putText(img, text, (50,50), 2,0.8, (0,0,0),2,cv2.LINE_AA)

	if cv2.waitKey(20) & 0xFF == 27:
		break

cv2.destroyAllWindows()
